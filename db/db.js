...
//var mongoConnectString = "mongodb://localhost/meancms";
var mongoConnectString = "mongodb://";//localhost/meancms";
if (process.env.MEAN_CMS_DATABASE_NAME && process.env.MEAN_CMS_DATABASE_USERNAME) {
    //database information is set in system variables and uses authentication
    mongoConnectString = (process.env.MEAN_CMS_DATABASE_USERNAME + ":" +
            process.env.MEAN_CMS_DATABASE_PASSWORD + "@" +
            process.env.MEAN_CMS_DATABASE_HOST + ':' +
            process.env.MEAN_CMS_DATABASE_PORT + '/' +
            process.env.MEAN_CMS_DATABASE_NAME);

} else if (process.env.MEAN_CMS_DATABASE_NAME) {
    mongoConnectString += (conf.MEAN_CMS_DATABASE_HOST + "/" + conf.MEAN_CMS_DATABASE_NAME);
}else {
    //default database
                ...
                if (err) {
                    ...
                    if (results.length === 0) {
                        ...
                        var adminUserRecord = {
                            ...
                            password: hashedPw,
                            enabled: true,
                            emailAddress: "admin@meancms.com",
                            firstName: "super",
